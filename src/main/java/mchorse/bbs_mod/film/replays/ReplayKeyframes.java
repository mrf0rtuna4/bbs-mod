...
public class ReplayKeyframes extends ValueGroup
{
    ...
    public void record(int tick, LivingEntity entity, List<String> groups)
    {
        ...
        if (position)
        {
            ...
            this.fall.insert(tick, entity.fallDistance);
        }
     ...
     */
    public void apply(int tick, LivingEntity entity, List<String> groups)
    {
        boolean empty = groups == null || groups.isEmpty();
        boolean position = empty || !groups.contains(GROUP_POSITION);
        boolean rotation = empty || !groups.contains(GROUP_ROTATION);
        ...
        if (position)
        {
            ...
            entity.fallDistance = (float) this.fall.interpolate(tick);
        }
        ...
        boolean sneaking = this.sneaking.interpolate(tick) != 0D;
        entity.setSneaking(sneaking);
        entity.setPose(sneaking ? EntityPose.CROUCHING : EntityPose.STANDING);
